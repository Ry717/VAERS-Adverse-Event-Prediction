...
# 讀取 SVM 的紀錄檔
trials_df = pd.read_csv('svm_pca_aucpr_optuna.csv')
best_trial = trials_df.sort_values(by='value', ascending=False).iloc[0]

best_kernel = str(best_trial.get('kernel', 'rbf'))

raw_gamma = best_trial.get('gamma', 'scale')
...
raw_cw = best_trial.get('class_weight', None)
...
# 建立 SVM 模型 (必須開啟 probability=True 才能畫曲線)
clf = SVC(
    **best_params,
    probability=True, 
    random_state=42
...
ax1.plot(fpr, tpr, color='crimson', lw=2.5, label=f'PCA+SVM (Baseline) (AUC = {roc_auc:.4f})')
...
ax2.plot(recall, precision, color='forestgreen', lw=2.5, label=f'PCA+SVM (Baseline) (AUC-PR = {pr_auc:.4f})')
...
plt.tight_layout()
output_filename = 'final_svm_pca_baseline.png' 
