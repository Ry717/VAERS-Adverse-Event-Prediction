...
trials_df = pd.read_csv('svm_rvkde_dual_aucpr_optuna.csv')
...
roc_auc = auc(fpr, tpr)
ax1.plot(fpr, tpr, color='crimson', lw=2.5, label=f'RVKDE(dual)+SVM (AUC = {roc_auc:.4f})')
...
ax2.plot(recall, precision, color='forestgreen', lw=2.5, label=f'RVKDE(dual)+SVM (AUC-PR = {pr_auc:.4f})')
...
plt.tight_layout()
output_filename = 'final_svm_rvkde_dual.png' 
