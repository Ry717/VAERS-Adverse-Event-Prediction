...
output_file = open('svm_rvkde_dual_optuna_output.txt', 'w', encoding='utf-8-sig')
...
Y = (merged_df[target_columns] == 1).any(axis=1).astype(int).values # 轉為 numpy array
...
# 將訓練集依據標籤拆分為危險(1)與安全(0)兩群
X_train_pos = X_train_text[y_train == 1]
X_train_neg = X_train_text[y_train == 0]

print(f"正類核心數: {len(X_train_pos)}, 負類核心數: {len(X_train_neg)}")

# 建立正類與負類的近鄰樹與頻寬
nn_pos = RVKDE.build_nn_kernels(X_train_pos, K2=K2+1, backend=backend_choice)
sigmas_pos, _ = RVKDE.rvkde_sigmas(X_train_pos, beta=1.0, dim=dim_pca, backend=backend_choice)

nn_neg = RVKDE.build_nn_kernels(X_train_neg, K2=K2+1, backend=backend_choice)
sigmas_neg, _ = RVKDE.rvkde_sigmas(X_train_neg, beta=1.0, dim=dim_pca, backend=backend_choice)

pdf_train_pos = RVKDE.cross_group_density_pairwise(
    X_query=X_train_text, X_kernels=X_train_pos, sigmas_k=sigmas_pos, nn=nn_pos, K2=K2, dim=dim_pca, same_dataset=False
).reshape(-1, 1)

pdf_train_neg = RVKDE.cross_group_density_pairwise(
    X_query=X_train_text, X_kernels=X_train_neg, sigmas_k=sigmas_neg, nn=nn_neg, K2=K2, dim=dim_pca, same_dataset=False
).reshape(-1, 1)

pdf_test_pos = RVKDE.cross_group_density_pairwise(
    X_query=X_test_text, X_kernels=X_train_pos, sigmas_k=sigmas_pos, nn=nn_pos, K2=K2, dim=dim_pca, same_dataset=False
).reshape(-1, 1)

pdf_test_neg = RVKDE.cross_group_density_pairwise(
    X_query=X_test_text, X_kernels=X_train_neg, sigmas_k=sigmas_neg, nn=nn_neg, K2=K2, dim=dim_pca, same_dataset=False
).reshape(-1, 1)

print("組合最終特徵並進行欠採樣...")
X_train_final = np.hstack([pdf_train_pos, pdf_train_neg, age_sex_train])
X_test_final = np.hstack([pdf_test_pos, pdf_test_neg, age_sex_test])
print(f"SVM 模型的特徵剩：{X_train_final.shape[1]} 維 (PDF_Pos + PDF_Neg + 年齡 + 2*性別)\n")

rus = RandomUnderSampler(sampling_strategy=1.0, random_state=42)
X_train_resampled, y_train_resampled = rus.fit_resample(X_train_final, y_train)

print("進行 SVM + RVKDE(dual) 超參數優化...\n")
...
csv_filename = "svm_rvkde_dual_aucpr_optuna.csv"
