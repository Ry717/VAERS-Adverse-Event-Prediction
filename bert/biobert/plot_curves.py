...
plt.tight_layout()
shap_filename = 'shap_summary.png'
plt.savefig(shap_filename, dpi=300, bbox_inches='tight')
print(f"SHAP 圖表已成功儲存為：【{shap_filename}】")
...
if isinstance(shap_values, list):
    shap_vals_for_ranking = shap_values[1]
else:
    shap_vals_for_ranking = shap_values

mean_abs_shap = np.abs(shap_vals_for_ranking).mean(axis=0)
top_20_features = mean_abs_shap.argsort()[-20:][::-1]

text_feature_count = X_vector.shape[1]
for rank_idx, feature_id in enumerate(top_20_features, 1):
    print(f"【Feature {feature_id}】")
    
    if feature_id < text_feature_count:
        feature_values = X_vector[:, feature_id]
        top_patient_indices = feature_values.argsort()[-3:][::-1]
        
        for local_rank, idx in enumerate(top_patient_indices, 1):
            score = feature_values[idx]
            text = df.iloc[idx]['SYMPTOM_TEXT']
            print(f" 嚴重度第 {local_rank} 名 (分數: {score:.4f}):")
            print(f"     {text}")
    else:
        # 這是傳統變數 (年齡或性別)
        if feature_id == text_feature_count:
            feature_name = "年齡 (AGE_YRS)"
        else:
            feature_name = "性別 (SEX_OneHot)"
            
        print(f" 這是傳統變數：【{feature_name}】")
        print("  (這不是病歷文字，因此無須印出內容)\n")
    print("\n")
